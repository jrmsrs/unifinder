<script lang="ts">
  ...
  import { ArrowLeft, AtSign, Calendar, CheckCircle, Clock, MapPin } from 'lucide-svelte';
...
<Modal
  class="
    w-11/12 max-w-sm text-gray-900 shadow-2xl shadow-black backdrop:bg-transparent backdrop:backdrop-blur-sm 
    sm:max-w-md md:max-w-3xl xl:max-w-4xl
    dark:text-white [&>*]:grid [&>*]:grid-cols-5 [&>*]:gap-4 [&>*]:p-4
  "
  size="lg"
  permanent={true}
  dismissable={false}
...
>
    ...
    {:then { objeto }}
      <div class="flex flex-col">
        ...
        <ObjetoTable>
          ...
          <Row key="Coletado por:" value={objeto.usuario.username} icon={AtSign} />
          ...
          <Row
            key="Status:"
            value={objeto.status === 'FINALIZADO' ? 'Finalizado' : 'Em andamento'}
            icon={objeto.status === 'FINALIZADO' ? CheckCircle : Clock}
          />
        </ObjetoTable>
      </div>
    {:catch error}
  ...
  <div class="g-f col-span-5 flex flex-col gap-4 md:col-span-2 [&>div]:rounded-lg [&>div]:bg-gray-100 [&>div]:p-4 [&>div]:dark:bg-gray-900">
    <div id="acoes">
